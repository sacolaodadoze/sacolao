...
const isValidHour = useCallback((dateStr, hourStr) => {
  ...
  if (isToday) {
    const now     = new Date();
    const nowMins = now.getHours() * 60 + now.getMinutes() + settings.minimum_schedule_minutes;
    if (hourMins < nowMins) return false;

    if (settings.minimum_hour_to_schedule_same_day) {
      const [lh, lm] = settings.minimum_hour_to_schedule_same_day.split(":").map(Number);
  ...
  if (isSaturday) {
    return inRange(settings.saturday_open_delivery, settings.saturday_close_delivery);
  }

  // día de semana — válido si cabe en mañana O en tarde
  return (
    inRange(settings.weekday_delivery_open_morning,   settings.weekday_delivery_close_morning) ||
    inRange(settings.weekday_delivery_open_afternoon, settings.weekday_delivery_close_afternoon)
  );
}, [settings]);
...
// en useDeliverySlots.js agregás slotsByDate
const slotsByDate = useMemo(() => {
  ...
  availableDates.forEach(({ dateKey }) => {
    ...
    const generateSlots = (open, close) => {
      ...
      const nowMins = now
        ? now.getHours() * 60 + now.getMinutes() + settings.minimum_schedule_minutes
        : 0;

      for (let t = openMins; t + settings.delivery_window_minutes <= closeMins; t += settings.delivery_window_minutes) {
    ...
    if (isSaturday) {
      generateSlots(settings.saturday_open_delivery, settings.saturday_close_delivery);
    } else {
      generateSlots(settings.weekday_delivery_open_morning,   settings.weekday_delivery_close_morning);
      generateSlots(settings.weekday_delivery_open_afternoon, settings.weekday_delivery_close_afternoon);
    }
...
return { isLoading, availableDates, slotsByDate, isValidHour,settingsDelivery :settings/* getEstimatedHour */ };
;
}